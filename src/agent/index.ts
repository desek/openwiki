...
import { ChatClaudeAgentSdkModel } from "./claude-agent-sdk.js";
...
export async function runOpenWikiAgent(
  ...
  warnOnAnthropicApiKeyFootgun(provider, options);
  ...
  try {
    ...
    throw translateAnthropicCategorical429(error, provider, modelId);
  } finally {
...
function ensureProviderKey(provider: OpenWikiProvider): void {
  ...
  // FR-7: this guard fires before `createModel` constructs the adapter, so it is
  // the error the user actually sees when the token is missing. The generic
  // message names the variable but not the remedy, so `anthropic-claude` is
  // special-cased to also reference `claude setup-token`.
  if (provider === "anthropic-claude") {
    throw new Error(
      `${CLAUDE_CODE_OAUTH_TOKEN_ENV_KEY} is required to run OpenWiki with ${getProviderLabel(provider)}. Generate one by running \`claude setup-token\` and paste it during onboarding.`,
    );
  }
...
/**
 * Emits the FR-6 precedence warning when the `anthropic-claude` provider is
 * active while a stale `ANTHROPIC_API_KEY` is also present. The adapter scrubs
 * the metered key from the Agent SDK environment; this warning makes the
 * silent-fallback footgun visible so the user knows the subscription OAuth token
 * is the credential actually in use.
 *
...
function warnOnAnthropicApiKeyFootgun(
  ...
  if (
    ...
    !process.env[ANTHROPIC_API_KEY_ENV_KEY]
  ) {
    return;
  }

  const warning =
    `Warning: ${ANTHROPIC_API_KEY_ENV_KEY} is set but will be ignored. The ` +
    `anthropic-claude provider authenticates with the subscription OAuth token ` +
    `${CLAUDE_CODE_OAUTH_TOKEN_ENV_KEY}, which is being used instead of the API key.`;
  options.onEvent?.({ type: "text", text: warning });
  emitDebug(options, `provider.footgun ${ANTHROPIC_API_KEY_ENV_KEY} ignored`);
}

/**
 * Translates a categorical `429` raised by the raw `anthropic` (API-key)
 * provider on a non-Haiku model into FR-8 guidance. A subscription token used on
 * the raw Messages API is capped to Haiku, so Sonnet/Opus/Fable return a
 * categorical `429 rate_limit_error` with no `retry-after`. Rather than leak the
 * opaque rate-limit error, the run points the user to `anthropic-claude` with
 * `CLAUDE_CODE_OAUTH_TOKEN`, the only sanctioned path to the full lineup.
 *
 * @param error - The error thrown from the run.
 * @param provider - The active provider (only `anthropic` is translated).
...
function translateAnthropicCategorical429(
  ...
  if (
    provider !== "anthropic" ||
    isHaikuModelId(modelId) ||
    !isCategorical429Error(error)
  ) {
    return error;
  }

  return new Error(
    `The Anthropic API returned a categorical 429 (rate_limit_error) for model ` +
      `"${modelId}". A Claude subscription OAuth token used on the raw Anthropic ` +
      `API path is capped to Haiku; Sonnet, Opus, and Fable are rejected there. ` +
      `To use "${modelId}", switch to the anthropic-claude provider ` +
      `(OPENWIKI_PROVIDER=anthropic-claude) and authenticate with ` +
      `${CLAUDE_CODE_OAUTH_TOKEN_ENV_KEY}, generated by \`claude setup-token\`, ` +
      `which routes through the Agent SDK path that unlocks the full Claude lineup.`,
    { cause: error },
  );
}

/**
 * Determines whether a model id names a Haiku model, which the raw Anthropic
 * path serves for subscription tokens and therefore must not trigger the FR-8
 * translation.
 *
 * @param modelId - The requested model id.
 * @returns `true` when the id refers to a Haiku model.
 */
function isHaikuModelId(modelId: string): boolean {
  return modelId.toLowerCase().includes("haiku");
}

/**
 * Recognizes a categorical `429` rate-limit failure across the shapes an
 * Anthropic/LangChain error can take (a `status`/`statusCode` field or a
 * message mentioning a 429 rate-limit).
 *
 * @param error - The thrown error.
 * @returns `true` when the error represents a `429` rate-limit condition.
 */
function isCategorical429Error(error: unknown): boolean {
  if (!isRecord(error)) {
    return false;
  }
...
function createModel(
  ...
  if (provider === "anthropic-claude") {
    // FR-2: route subscription inference through the Agent SDK adapter, never
    // the raw Messages API. The adapter authenticates with
    // CLAUDE_CODE_OAUTH_TOKEN and scrubs ANTHROPIC_API_KEY from the SDK env.
    return new ChatClaudeAgentSdkModel({
      model: modelId,
      ...retryOptions,
    });
  }
